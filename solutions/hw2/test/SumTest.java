...
public final class SumTest {
    ...
    private static final BiConsumer<Number, String> TO_STRING = (expected, out) -> Asserts.assertEquals("hw2.Sum", expected.toString(), out);
    ...
    /* package-private */ static <T extends Number> Consumer<TestCounter> variant(
        ...
        return counter -> modifier.getValue().apply(test.getValue().get())
                .test("hw2.Sum" + test.getName() + modifier.getName() + runner.getName(), counter, runner.getValue());
    }
    ...
    private static BiConsumer<Number, String> approximate(final Function<String, Number> parser, final double precision) {
        return (expected, out) ->
                Asserts.assertEquals("hw2.Sum", expected.doubleValue(), parser.apply(out).doubleValue(), precision);
    }

    /* package-private */ static final Named<Function<String, Runner>> RUNNER =
            Named.of("", Runner.packages("", "hw2")::args);
